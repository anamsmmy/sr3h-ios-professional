...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _startConversion() async {
    ...
    try {
      ...
      final extension = _selectedVideoName!.split('.').last.toLowerCase();
      
      // Ensure output is MP4
      final outputPath = '${outputDir.path}/$fileName-SR3H.mp4';

      setState(() {
        _conversionProgress = 'جاري تحويل الفيديو إلى 60 إطار في الثانية...';
      ...
      await FFmpegKit.executeAsync(command, (Session session) async {
        ...
        if (message.contains('time=')) {
          setState(() {
            ...
            _conversionProgressPercent = 0.5; // تقدير تقريبي
          });
        }
        ...
        if (statistics.getTime() > 0) {
          setState(() {
            _conversionProgressPercent = 0.7; // تقدير تقريبي
          });
        }
      });
