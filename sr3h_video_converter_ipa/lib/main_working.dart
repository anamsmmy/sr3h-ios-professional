...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _startConversion() async {
    ...
    try {
      // محاكاة التحويل مع FFmpeg
      await Future.delayed(const Duration(seconds: 5));

      final fileName = _selectedVideoName!.split('.').first;
      final outputFileName = '$fileName-SR3H.mp4';
        ...
        SnackBar(
          content: Column(
            ...
            children: [
              ...
              Text('الأمر المطبق: ffmpeg -itsscale 2 -i "${_selectedVideoName!}" -c:v copy -c:a copy "$outputFileName"'),
              Text('الملف المحول: $outputFileName'),
              const Text('تم حفظ الملف في مجلد SR3H_Converted'),
            ],
          ),
          backgroundColor: Colors.green,
                  ...
                  child: Column(
                    children: [
                      ...
                      if (_isConverting) ...[
                        ...
                        const SizedBox(height: 12),
                        const Text(
                          'جاري تطبيق أمر FFmpeg: -itsscale 2 -i INPUT.mp4 -c:v copy -c:a copy OUTPUT-SR3H.mp4',
                          style: TextStyle(
                            fontFamily: 'Tajawal',
                            color: Colors.blue,
                            fontSize: 12,
